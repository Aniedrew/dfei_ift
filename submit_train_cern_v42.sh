#!/bin/bash
#
# DFEI CERN v42 训练 (PV 分簇: 可训练 cluster 头 + 温度退火 + 课程过渡 + val 对齐)
# v40 失败修复: ①val 也切子图 ②truth->cluster 课程式过渡 ③训练侧子图数上限
#
# 提交方式:
#   hep_sub submit_train_cern_v42.sh -g ghigh -gpu 1 -cpu 4 -m 32000 -wt long \
#       -o logs/train_cern_v42.out -e logs/train_cern_v42.err
#
...
echo "CONFIG      : config_files/train_CERN_v42.yaml"
echo "========================================"
...
if grep -qE 'resume_ckpt:\s*".*TODO' config_files/train_CERN_v42.yaml; then
  echo "[CHECK] FAIL: train_CERN_v42.yaml 的 resume_ckpt 仍是 TODO, 先填 v38 best checkpoint"
  exit 1
fi
...
if [ $PREFLIGHT_RC -ne 0 ]; then
  ...
  MAX_RETRY=1000
  RETRY_SLEEP=60
  RETRY_COUNT_FILE=/lzufs/home/guoqingxiang/dfei/scalable_mtl_hgnn/logs/v42.retry_count
  N=0
  ...
  if [ "$N" -lt "$MAX_RETRY" ]; then
    ...
    sleep $RETRY_SLEEP
    echo "[RETRY] 重新提交 submit_train_cern_v42.sh (原作业 ${_CONDOR_IHEP_JOB_ID:-unknown}, GPU ${CUDA_VISIBLE_DEVICES})"
    hep_sub submit_train_cern_v42.sh -g ghigh -gpu 1 -cpu 4 -m 32000 -wt long -o logs/train_cern_v42.out -e logs/train_cern_v42.err
    echo "[RETRY] 已重提, 本次退出"
    exit 0
  fi
  ...
  curl -s --connect-timeout 10 -X POST https://sctapi.ftqq.com/SCT387631TDiuLj6UNUsFTaDRjkaSWcdPv.send \
    -d "title=[DFEI] ⚠️ v42训练 Job ${JOB_ID} 分到坏GPU" \
    -d "desp=## v42训练作业 ${JOB_ID} GPU预检失败(重试${MAX_RETRY}次后仍失败)
| 作业 | ${JOB_ID} |
| 主机 | $(hostname) |
...
# 标记训练已真正开始
touch /lzufs/home/guoqingxiang/dfei/scalable_mtl_hgnn/logs/v42_started.flag

python3 -u wmpgnn/analysis/trainer.py --config config_files/train_CERN_v42.yaml
...
curl -s --connect-timeout 10 -X POST https://sctapi.ftqq.com/SCT387631TDiuLj6UNUsFTaDRjkaSWcdPv.send \
  -d "title=[DFEI] ${STATUS} v42训练 Job ${JOB_ID}" \
  -d "desp=## v42训练作业 ${JOB_ID} ${STATUS}
...
| **状态** | ${STATUS} |
| **主机** | $(hostname) |
| **配置** | train_CERN_v42.yaml (PV分簇: 小规模快速验证, 50files/50ep) |
| **结束时间** | $(date) |
| **退出码** | ${EXIT_CODE} |

### 查看日志
\`\`\`bash
tail -50 /lzufs/home/guoqingxiang/dfei/scalable_mtl_hgnn/logs/train_cern_v42.out
\`\`\`" > /dev/null 2>&1
