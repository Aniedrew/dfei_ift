...
echo "CONFIG      : config_files/train_public_v38.yaml"
echo "========================================"
...
if grep -qE 'resume_ckpt:\s*".*TODO' config_files/train_public_v38.yaml; then
  echo "[CHECK] FAIL: train_public_v38.yaml 的 resume_ckpt 仍是 TODO, 先填 v38 best checkpoint"
  exit 1
fi
...
if [ $PREFLIGHT_RC -ne 0 ]; then
  ...
  MAX_RETRY=1000
  RETRY_SLEEP=60
  RETRY_COUNT_FILE=/lzufs/home/guoqingxiang/dfei/scalable_mtl_hgnn/logs/public_v38.retry_count
  N=0
  ...
  if [ "$N" -lt "$MAX_RETRY" ]; then
    ...
    sleep $RETRY_SLEEP
    echo "[RETRY] 重新提交 submit_train_public_v38.sh (原作业 ${_CONDOR_IHEP_JOB_ID:-unknown}, GPU ${CUDA_VISIBLE_DEVICES})"
    hep_sub submit_train_public_v38.sh -g ghigh -gpu 1 -cpu 4 -m 32000 -wt long -o logs/train_public_v38.out -e logs/train_public_v38.err
    echo "[RETRY] 已重提, 本次退出"
    exit 0
  fi
...
# 标记训练已真正开始
touch /lzufs/home/guoqingxiang/dfei/scalable_mtl_hgnn/logs/public_v38_started.flag

python3 -u wmpgnn/analysis/trainer.py --config config_files/train_public_v38.yaml
...
| **状态** | ${STATUS} |
| **主机** | $(hostname) |
| **配置** | train_public_v38.yaml (PV分簇: 小规模快速验证, 50files/50ep) |
| **结束时间** | $(date) |
| **退出码** | ${EXIT_CODE} |
