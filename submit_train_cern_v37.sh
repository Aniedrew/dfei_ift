...
#
# 提交方式:
#   hep_sub submit_train_v37.sh -g ghigh -gpu 1 -cpu 4 -m 32000 -wt long \
#       -o logs/train_v37.out -e logs/train_v37.err
#
...
echo "CONFIG      : config_files/train_CERN_v37.yaml"
echo "========================================"
...
if grep -q "待定" config_files/train_CERN_v37.yaml; then
  echo "[CHECK] WARN: train_CERN_v37.yaml 仍含 '待定' 注释, 请确认参数已确定"
fi
...
if [ $PREFLIGHT_RC -ne 0 ]; then
  ...
  MAX_RETRY=1000
  RETRY_SLEEP=60
  RETRY_COUNT_FILE=/lzufs/home/guoqingxiang/dfei/scalable_mtl_hgnn/logs/v37.retry_count
  N=0
  ...
  if [ "$N" -lt "$MAX_RETRY" ]; then
    ...
    sleep $RETRY_SLEEP
    echo "[RETRY] 重新提交 submit_train_cern_v37.sh (原作业 ${_CONDOR_IHEP_JOB_ID:-unknown}, GPU ${CUDA_VISIBLE_DEVICES})"
    hep_sub submit_train_cern_v37.sh -g ghigh -gpu 1 -cpu 4 -m 32000 -wt long -o logs/train_cern_v37.out -e logs/train_cern_v37.err
    echo "[RETRY] 已重提, 本次退出"
    exit 0
  fi
  ...
  curl -s --connect-timeout 10 -X POST https://sctapi.ftqq.com/SCT387631TDiuLj6UNUsFTaDRjkaSWcdPv.send \
    ...
    -d "desp=## Next训练作业 ${JOB_ID} GPU预检失败(重试${MAX_RETRY}次后仍失败)
| 作业 | ${JOB_ID} |
| 主机 | $(hostname) |
...
# 标记训练已真正开始
touch /lzufs/home/guoqingxiang/dfei/scalable_mtl_hgnn/logs/v37_started.flag

python3 -u wmpgnn/analysis/trainer.py --config config_files/train_CERN_v37.yaml
...
| **状态** | ${STATUS} |
| **主机** | $(hostname) |
| **配置** | train_CERN_v37.yaml (B2+源检测头+loss再平衡) |
| **结束时间** | $(date) |
| **退出码** | ${EXIT_CODE} |

### 查看日志
\`\`\`bash
tail -50 /lzufs/home/guoqingxiang/dfei/scalable_mtl_hgnn/logs/train_v37.out
\`\`\`" > /dev/null 2>&1
